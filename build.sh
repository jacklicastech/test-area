...
if [ "x$NOS3" = "x" ]
then
  aws s3 sync $artifacts_dir s3://castlestw/luna/local/$target
fi

cd luna
aws s3 cp s3://castlestw/luna/local/$target/index.json ./index.json
for artifact in $artifacts_names
do
  if [ "x$artifact" != "xluna.tar.gz" ]
  then
    rake update_index PREFIX=luna/local/$target KEY=$artifact
  fi
done
aws s3 cp ./index.json s3://castlestw/luna/local/$target/index.json
